...
//  SuperDB
//
//  Created by Maurice Achtenhagen on 9/2/16.
//  Copyright © 2016 Maurice Achtenhagen. All rights reserved.
//
...
class HeroListController: UIViewController, UITableViewDataSource,
	UITableViewDelegate, UITabBarDelegate, NSFetchedResultsControllerDelegate, UIAlertViewDelegate {
	...
	override func viewDidLoad() {
		...
		do {
			...
			let title = NSLocalizedString("Error performing fetch", comment: "Error performing fetch")
			let message = NSLocalizedString("Error was: \(error.description), quitting", comment: "Error was: \(error.description), quitting")
			showAlertWithCompletion(title, message: message, completion: { _ in exit(-1) })
		}
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
		...
		let tab = tableArray.indexOfObject(self.heroTabBar.selectedItem!)
